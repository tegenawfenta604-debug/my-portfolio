// particles.js
// Three.js glowing particle network for the hero (ES module).
// Exports initHeroParticles(canvas). Uses BufferGeometry + Points
// (GPU friendly) with a neural-network of connecting lines,
// mouse parallax, and a GSAP intro timeline.
...
export function initHeroParticles(canvas) {
    ...
    try {
        ...
        // WebGL unavailable -> the light CSS background remains.
        return;
    }
    ...
    // Soft circular glow sprite (simulates bloom on a light background)
    function makeGlow() {
    ...
    // Neon palette
    const palette = [0x6c47ff, 0x00c2ff, 0xb388ff, 0x9b5cff];
    ...
    if (window.gsap) {
        ...
        tl.to(pMat, { opacity: 0.95, duration: 2.6 }, 0.6);
        tl.to(lMat, { opacity: 0.22, duration: 2.6 }, 1.0);
